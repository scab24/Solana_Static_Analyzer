...
use syn::{
    File, Ident, Item, ItemFn, ItemStruct,
    visit::{self, Visit},
};
        ...
        .query(|ast| {
            ...
            for node in visitor.nodes {
                ...
                let description = if name.starts_with("function_") {
                    ...
                    format!(
                        "The function '{}' does not follow the recommended snake_case convention",
                        function_name
                    )
                } else if name.starts_with("struct_") {
                    let struct_name = name.strip_prefix("struct_").unwrap_or(name);
                    format!(
                        "The struct '{}' does not follow the recommended PascalCase convention",
                        struct_name
                    )
                } else {
                    format!(
                        "The identifier '{}' does not follow the recommended naming conventions",
                        name
                    )
                };
